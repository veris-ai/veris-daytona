  ...
  return async (input: ExperimentalChatSystemTransformInput, output: ExperimentalChatSystemTransformOutput) => {
    output.system.push(
      [
        ...
        'than this one.',
        '',
        'TLS trust is already configured. `SSL_CERT_FILE`, `REQUESTS_CA_BUNDLE`,',
        '`CURL_CA_BUNDLE` and the other CA variables point at a bundle carrying the public',
        'roots plus the gateway CA, and `NODE_OPTIONS` carries `--use-openssl-ca` so Node',
        'reads that same bundle (Daytona repoints `NODE_EXTRA_CA_CERTS` at its own file;',
        'that is expected and not a problem to fix). Plain `node`, `curl`, `python`, `pip`',
        'and `npm` verify vendor hosts as-is. Do NOT prefix commands with',
