  ...
  return async (input: ExperimentalChatSystemTransformInput, output: ExperimentalChatSystemTransformOutput) => {
    output.system.push(
      [
        ...
        'than this one.',
        '',
        'TLS trust is already configured. The CA variables in the environment, including',
        '`NODE_EXTRA_CA_CERTS`, name files that already carry the gateway CA (Daytona',
        'points them at its own file; the gateway CA was added to it at setup — expected,',
        'not a problem to fix). Plain `node`, `curl`, `python`, `pip` and `npm` verify',
        'vendor hosts as-is. Do NOT prefix commands with',
